...
var getMessageList = function(request, response) {
  ...
  collection.find({}, { sort: { "meta.upDateAt": -1 } }, function(err, docs) {
    if (err) {
      response.json({ result: "fail", errMessage: err });
    } else {
      response.json({ result: "success", errMessage: err, messageList: docs });
    }
  });
};
...
var getMessageListByPage = function(request, response) {
  ...
  if (page) {
    collection.find(
      {},
      { sort: { "meta.upDateAt": -1 }, skip: start, limit: size },
      function(err, docs) {
        if (err) {
          response.json({ result: "fail", errMessage: err });
        } else {
...
// -------------------------------------------
var getDayList = function(request, response) {
  ...
  if (code == "666") {
    ...
    response.json({ result: "fail", errMessage: "输入效验码" });
  }
};
app.get("/getDayList", getDayList);
    ...
    collection.insert(
      {
        ...
        title: title,
        content: content,
        meta: { createAt: Date.now(), upDateAt: Date.now() }
      },
      function(err, doc) {
        if (err) throw err;
...
// ---------------------------------------
var addDay = function(request, response) {
  var collection = db.get("day");
  console.log(request.query);
  var name = request.query["name"];
  var title = request.query["title"];
  ...
  var ywc = request.query["ywc"];
  var wwc = request.query["wwc"];
  var xt = request.query["xt"];
  if (name && title && date && ywc && wwc && xt) {
    collection.insert(
      { date: date, name: name, title: title, ywc: ywc, wwc: wwc, xt: xt },
      function(err, doc) {
        if (err) throw err;
        response.json({ result: "success" });
      }
...
var getPraise = function(request, response) {
  ...
  if (id && icon && name) {
    collection.find({ id: id }, function(err, doc) {
      ...
      console.log(icon);
      console.log(name);
      collection.find({ id: id, icon: icon, name: name }, {}, function(
        err,
        doc
      ) {
        if (err) throw err;
